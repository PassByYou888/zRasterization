...
procedure TFontBuildForm.BuildButtonClick(Sender: TObject);
  ...
  function computeYHead(R: TMZR): Integer;
  var
    X, Y: Integer;
  begin
  ...
  function computeYTail(R: TMZR; sp: Integer): Integer;
  var
    X, Y: Integer;
  begin
  ...
  begin
    ...
    if (GR.width > 0) and (GR.height > 0) then
      begin
        ...
        N2 := TMZR.Create;
        N2.DrawMode := ZR.MemoryRaster.TDrawMode.dmBlend;
        ...
        if EX - BX > 0 then
          begin
            n := TMZR.Create;
            n.SetSize(EX - BX, GR.height, ZRColor(0, 0, 0, 0));
...
begin
  ...
  raster := TMZR.Create;
  raster.DrawMode := ZR.MemoryRaster.TDrawMode.dmBlend;
