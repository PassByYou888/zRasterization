...
uses
  ZR.h264.Types, ZR.h264.Stats, ZR.FPC.GenericList, ZR.Core, ZR.MemoryRaster, Math;
...
procedure YV12ToZR(const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h, stride, stride_cr: int32_t; const dest: TMZR; const forceITU_BT_709, lumaFull: Boolean); overload;
procedure ZRToYV12(const sour: TMZR; const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h: int32_t); overload;
...
procedure YV12ToZR(const luma_ptr, u_ptr, v_ptr: uint8_p; const w, h, stride, stride_cr: int32_t; const dest: TMZR; const forceITU_BT_709, lumaFull: Boolean);
...
begin
  ...
  for y := 0 to dest.height shr 1 - 1 do
    begin
      row1 := PZRColorEntry(dest.ScanLine[y * 2]);
      row2 := PZRColorEntry(dest.ScanLine[y * 2 + 1]);
