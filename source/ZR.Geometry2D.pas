...
{$IFDEF FPC}
  ZR.FPC.GenericList,
{$ENDIF FPC}
  ZR.Core, Types, ZR.MemoryStream, ZR.PascalStrings, ZR.UPascalStrings;
...
{$REGION 'Nearest_Box_Tool'}
  ...
  TNearest_Box_IoU_Tool_ = TZR_BL<TNearest_Box_IoU>;
  ...
  TNearest_Box_Tool_ = TZR_BL<TNearest_Box_Data>;

  TNearest_Box_List_ = TZR_BL<PNearest_Box_Data>;
...
uses ZR.UnicodeMixedLib, ZR.Geometry3D, ZR.Geometry.Low, ZR.DFE, ZR.Status;
...
{$I ZR.Geometry.Split.Header.inc}
{$I ZR.Geometry.Split.Body.inc}
{$IFDEF FPC}
...
begin
  Result :=
    ZR.Geometry2D.IsZero(LeftTop) and
    ZR.Geometry2D.IsZero(RightTop) and
    ZR.Geometry2D.IsZero(RightBottom) and
    ZR.Geometry2D.IsZero(LeftBottom);
end;
...
begin
  Result := ZR.Geometry2D.BoundRect(LeftTop, RightTop, RightBottom, LeftBottom);
end;
...
begin
  if length(buff) <> 4 then
    begin
      Result := TV2Rect4.Init(ZR.Geometry2D.BoundRect(buff));
    end
  else
    with Result do
      begin
