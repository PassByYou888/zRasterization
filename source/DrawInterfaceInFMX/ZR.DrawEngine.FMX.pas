...
{$ENDIF}
  ZR.Geometry3D, ZR.ListEngine,
  ZR.PascalStrings,
  ZR.Core, ZR.DrawEngine, ZR.UnicodeMixedLib, ZR.Geometry2D, ZR.MemoryRaster;
...
function AlphaColor2RasterColor(c: TAlphaColor): TZRColor;
...
procedure MemoryBitmapToSurface(raster: TMZR; Surface: TBitmapSurface); overload;
procedure MemoryBitmapToSurface(raster: TMZR; sourRect: TRect; Surface: TBitmapSurface); overload;
procedure SurfaceToMemoryBitmap(Surface: TBitmapSurface; raster: TMZR);
procedure MemoryBitmapToBitmap(raster: TMZR; bmp: TBitmap); overload;
procedure MemoryBitmapToBitmap(raster: TMZR; sourRect: TRect; bmp: TBitmap); overload;
procedure BitmapToMemoryBitmap(bmp: TBitmap; raster: TMZR);

function CanLoadMemoryBitmap(f: SystemString): Boolean;
procedure LoadMemoryBitmap(f: SystemString; raster: TMZR); overload;
procedure LoadMemoryBitmap(f: SystemString; raster: TSequenceMemoryZR); overload;
procedure LoadMemoryBitmap(f: SystemString; raster: TDETexture); overload;
procedure LoadMemoryBitmap(stream: TCore_Stream; raster: TMZR); overload;

procedure SaveMemoryBitmap(f: SystemString; raster: TMZR); overload;
procedure SaveMemoryBitmap(raster: TMZR; fileExt: SystemString; DestStream: TCore_Stream); overload;
procedure SaveMemoryBitmap(raster: TSequenceMemoryZR; fileExt: SystemString; DestStream: TCore_Stream); overload;
...
uses ZR.Status, ZR.MemoryStream, ZR.MediaCenter;
...
procedure MemoryBitmapToSurface(raster: TMZR; Surface: TBitmapSurface);
var
  y, x: Integer;
  p1, p2: PCardinal;
  c: TZRColorEntry;
...
procedure MemoryBitmapToSurface(raster: TMZR; sourRect: TRect; Surface: TBitmapSurface);
var
  nb: TMZR;
begin
  nb := TMZR.Create;
  nb.DrawMode := dmBlend;
  nb.SetSize(sourRect.width, sourRect.height, ZRColor(0, 0, 0, 0));
  raster.ReadyBits;
      ...
      begin
        for x := 0 to Surface.width - 1 do
          with TAlphaColorRec(Surface.pixels[x, y]) do
              raster.Pixel[x, y] := ZRColor(r, g, b, a)
      end;
  except
  end;
end;

procedure MemoryBitmapToBitmap(raster: TMZR; bmp: TBitmap);
var
...
procedure MemoryBitmapToBitmap(raster: TMZR; sourRect: TRect; bmp: TBitmap);
var
  Surface: TBitmapSurface;
begin
  Surface := TBitmapSurface.Create;
...
procedure LoadMemoryBitmap(f: SystemString; raster: TMZR);
var
  Surf: TBitmapSurface;
begin
...
procedure LoadMemoryBitmap(stream: TCore_Stream; raster: TMZR);
var
  Surf: TBitmapSurface;
begin
...
procedure LoadMemoryBitmap(f: SystemString; raster: TSequenceMemoryZR);
begin
  if raster.CanLoadFile(f) then
      raster.LoadFromFile(f)
  else
      LoadMemoryBitmap(f, TMZR(raster));
end;

procedure LoadMemoryBitmap(f: SystemString; raster: TDETexture);
begin
  LoadMemoryBitmap(f, TSequenceMemoryZR(raster));
  raster.ReleaseGPUMemory;
end;

procedure SaveMemoryBitmap(f: SystemString; raster: TMZR);
var
  Surf: TBitmapSurface;
begin
...
procedure SaveMemoryBitmap(raster: TMZR; fileExt: SystemString; DestStream: TCore_Stream);
var
  Surf: TBitmapSurface;
begin
...
procedure SaveMemoryBitmap(raster: TSequenceMemoryZR; fileExt: SystemString; DestStream: TCore_Stream);
var
  Surf: TBitmapSurface;
begin
  if umlMultipleMatch(['.bmp'], fileExt) then
...
{$ENDIF}
  if FDebug then
    begin
      ...
      FCanvas.DrawRect(ZR.Geometry2D.TV2Rect4.Init(newDest.MakeRectV2, newDest.angle).BoundRectf, 0, 0, [], 0.3);
    end;
end;
...
begin
  ...
  DefaultTexture.SetSize(2, 2, ZRColorF(0, 0, 0, 1.0));
end;
...
procedure _SaveRaster(mr: TMZR; const fn: string);
begin
