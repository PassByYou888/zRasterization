...
uses Types,
  ZR.Core, ZR.UnicodeMixedLib, ZR.PascalStrings, ZR.UPascalStrings,
  ZR.ListEngine, ZR.MemoryStream, ZR.Cipher,
  ZR.Status, ZR.Geometry.Low, ZR.TextDataEngine, ZR.Geometry2D, ZR.Geometry3D,
  ZR.Json, ZR.Number,
{$IFDEF FPC}
  ZR.FPC.GenericList,
{$ELSE FPC}
  ZR.Delphi.JsonDataObjects,
{$ENDIF FPC}
  ZR.Compress;
...
uses SysUtils, Variants, ZR.Notify;
...
begin
  FOwner.ReadDF(FIndex, output);
  inc(FIndex);
end;

procedure TDFEReader.ReadDFE(output: TDFE);
begin
  FOwner.ReadDFE(FIndex, output);
  inc(FIndex);
end;
