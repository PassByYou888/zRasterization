...
uses Types, ZR.Core,
  ZR.Geometry.Low, ZR.Geometry2D, ZR.PascalStrings, ZR.UPascalStrings, ZR.UnicodeMixedLib;
...
begin
  Result.Buff := ZR.Geometry.Low.MatrixMultiply(Lhs.Buff, Rhs.Buff);
end;
...
begin
  Result.Buff := Buff;
  det := ZR.Geometry.Low.MatrixDeterminant(Result.Buff);
  if Abs(det) < Epsilon then
      Result.Buff := ZR.Geometry.Low.IdentityHmgMatrix
  else
    begin
      ZR.Geometry.Low.AdjointMatrix(Result.Buff);
      ZR.Geometry.Low.ScaleMatrix(Result.Buff, 1 / det);
    end;
end;
...
begin
  Result.Buff := Buff;
  ZR.Geometry.Low.TranslateMatrix(Result.Buff, v);
end;

function TMatrix4.Normalize: TMatrix4;
begin
  Result.Buff := Buff;
  ZR.Geometry.Low.NormalizeMatrix(Result.Buff);
end;

function TMatrix4.Transpose: TMatrix4;
begin
  Result.Buff := Buff;
  ZR.Geometry.Low.TransposeMatrix(Result.Buff);
end;

function TMatrix4.AnglePreservingInvert: TMatrix4;
begin
  Result.Buff := ZR.Geometry.Low.AnglePreservingMatrixInvert(Buff);
end;

function TMatrix4.Determinant: TGeoFloat;
begin
  Result := ZR.Geometry.Low.MatrixDeterminant(Buff);
end;
...
begin
  Result.Buff := ZR.Geometry.Low.Pitch(Buff, angle);
end;

function TMatrix4.Roll(angle: TGeoFloat): TMatrix4;
begin
  Result.Buff := ZR.Geometry.Low.Roll(Buff, angle);
end;

function TMatrix4.Turn(angle: TGeoFloat): TMatrix4;
begin
  Result.Buff := ZR.Geometry.Low.Turn(Buff, angle);
end;
...
begin
  Result.Buff := ZR.Geometry.Low.VectorTransform(Lhs.Buff, Rhs.Buff);
end;
...
begin
  Result.Buff := ZR.Geometry.Low.VectorTransform(Lhs.Buff, Rhs);
end;

class operator TVector4.Multiply(const Lhs: TMat4; const Rhs: TVector4): TVector4;
begin
  Result.Buff := ZR.Geometry.Low.VectorTransform(Rhs.Buff, Lhs);
end;
...
begin
  Result.Buff := ZR.Geometry.Low.VectorTransform(Rhs.Buff, Lhs.Buff);
end;
