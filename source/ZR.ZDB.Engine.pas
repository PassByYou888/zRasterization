...
uses SysUtils, Classes,
  ZR.ListEngine, ZR.PascalStrings, ZR.UPascalStrings, ZR.UnicodeMixedLib, ZR.TextDataEngine, ZR.Notify,
  ZR.Json,
  ZR.Core, ZR.MemoryStream, ZR.ZDB.ObjectData_LIB, ZR.ZDB,
  ZR.DFE, ZR.ZDB.ItemStream_LIB;
...
uses ZR.MH_ZDB, ZR.Cipher, ZR.Status;
  ...
  if M <> nil then
    begin
      ...
      try
        ...
        Result.MemoryUsed := ZR.MH_ZDB.GetHookMemorySize;
        if AllowedCache then
            ProcessNewInstanceCache(StorePos, Result, Result.MemoryUsed);
      finally
          ZR.MH_ZDB.EndMemoryHook;
      end;
    end;
end;
...
begin
  if AllowedCache then
    begin
      ...
      t.Buff := Buff;
      t.MemoryUsed := ZR.MH_ZDB.GetHookMemorySize;
      ZR.MH_ZDB.EndMemoryHook;
